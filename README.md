...
> ⚡️ Modern desktop project manager with a hardened Electron main process, a typed preload bridge, and a polished React/Redux experience (auth, Kanban, dashboards, theming, multilingual UI).
...
6. [🛠️ Scripts & Tooling](#-scripts--tooling)
...
* 🛰️ **Typed preload bridge** – `window.api` exposes health/auth/project/task IPC with DTO safety.
...
* 🛡️ Sandbox + `contextIsolation` + disabled `nodeIntegration` in `BrowserWindow`.
...
* ⚠️ **React Router warnings in tests** → opt into v7 future flags to silence upcoming behavior changes.
